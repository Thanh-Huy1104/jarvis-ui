...
export function useEve(apiEndpoint: string, isPersistent = false) {
    const [status, setStatus] = useState<EveStatus>('disconnected');
    const [messages, setMessages] = useState<Message[]>([]);
    const [analyserNode, setAnalyserNode] = useState<AnalyserNode | null>(null);

    // Refs for state
    const statusRef = useRef<EveStatus>('disconnected');
    ...
    const sessionIdRef = useRef<string>(`eve-session-${Date.now()}`);
    ...
    const startWakeWordListener = useCallback(() => {
        ...
        recognition.onresult = (event: any) => {
            ...
            if (transcript.includes("eve") || transcript.includes("hey eve")) {
